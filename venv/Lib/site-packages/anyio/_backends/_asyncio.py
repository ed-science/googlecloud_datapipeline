...
def find_root_task() -> asyncio.Task:
    ...
    if state := _task_states.get(task):
...
class TaskGroup(abc.TaskGroup):
    ...
    async def __aexit__(
        ...
        exceptions = (
            self._exceptions
            if self.cancel_scope._parent_cancelled()
            else self._filter_cancellation_errors(self._exceptions)
        )
    ...
    async with (limiter or current_default_thread_limiter()):
        with CancelScope(shield=not cancellable):
            ...
            if not idle_workers:
                ...
                while (
                    idle_workers
                    and not now - idle_workers[0].idle_since
...
async def create_udp_socket(
    ...
    return (
        ConnectedUDPSocket(transport, protocol)
        if remote_address
        else UDPSocket(transport, protocol)
    )
