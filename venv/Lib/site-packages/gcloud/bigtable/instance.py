...
_ADMIN_TYPE_URL_BASE = f'{_TYPE_URL_BASE}admin.v2.'
_INSTANCE_CREATE_METADATA = f'{_ADMIN_TYPE_URL_BASE}CreateInstanceMetadata'
_TYPE_URL_MAP = {
...
def _prepare_create_request(instance):
    ...
    """
    parent_name = f'projects/{instance._client.project}'
    ...
    cluster.name = f'{instance.name}/clusters/{instance.instance_id}'
    cluster.location = f'{parent_name}/locations/{instance._cluster_location_id}'
...
def _parse_pb_any_to_native(any_val, expected_type=None):
    ...
    if expected_type is not None and expected_type != any_val.type_url:
        raise ValueError(
            f'Expected type: {expected_type}, Received: {any_val.type_url}'
        )
...
class Operation(object):
    ...
    def __eq__(self, other):
        return (
            (
                other.op_type == self.op_type
                and other.op_id == self.op_id
                and other.begin == self.begin
                and other.location_id == self.location_id
                and other._instance == self._instance
                and other._complete == self._complete
            )
...
class Instance(object):
    ...
    * :meth:`undelete` itself
    ...
    def name(self):
        ...
        """
        return f'{self._client.project_name}/instances/{self.instance_id}'

    def __eq__(self, other):
        return (
            (
                other.instance_id == self.instance_id
                and other._client == self._client
            )
            if isinstance(other, self.__class__)
            else False
        )
    ...
    def list_clusters(self):
        ...
        failed_locations = list(list_clusters_response.failed_locations)
    ...
    def list_tables(self):
        ...
        for table_pb in table_list_pb.tables:
            table_prefix = f'{self.name}/tables/'
            if not table_pb.name.startswith(table_prefix):
                raise ValueError(f'Table name {table_pb.name} not of expected format')
