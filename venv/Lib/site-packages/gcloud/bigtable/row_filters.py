...
class _BoolFilter(RowFilter):
    ...
    def __eq__(self, other):
        return other.flag == self.flag if isinstance(other, self.__class__) else False
...
class ColumnRangeFilter(RowFilter):
    ...
    def to_pb(self):
        ...
        if self.end_column is not None:
            key = 'end_qualifier_closed' if self.inclusive_end else 'end_qualifier_open'
...
class ValueRangeFilter(RowFilter):
    ...
    def to_pb(self):
        ...
        if self.start_value is not None:
            key = 'start_value_closed' if self.inclusive_start else 'start_value_open'
            value_range_kwargs[key] = _to_bytes(self.start_value)
        if self.end_value is not None:
            key = 'end_value_closed' if self.inclusive_end else 'end_value_open'
...
class _CellCountFilter(RowFilter):
    ...
    def __eq__(self, other):
        return (
            other.num_cells == self.num_cells
            if isinstance(other, self.__class__)
            else False
        )
