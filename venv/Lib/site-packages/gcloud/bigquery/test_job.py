...
class _Base(object):
    ...
    def _setUpConstants(self):
        ...
        self.WHEN = datetime.datetime.utcfromtimestamp(self.WHEN_TS).replace(
            tzinfo=UTC)
        self.ETAG = 'ETAG'
        self.JOB_ID = f'{self.PROJECT}:{self.JOB_NAME}'
...
class TestLoadTableFromStorageJob(unittest2.TestCase, _Base):
    ...
    def test_ctor(self):
        ...
        self.assertEqual(job.path, f'/projects/{self.PROJECT}/jobs/{self.JOB_NAME}')
    ...
    def test_props_set_by_server(self):
        ...
        JOB_ID = f'{self.PROJECT}:{self.JOB_NAME}'
        URL = f'http://example.com/projects/{self.PROJECT}/jobs/{self.JOB_NAME}'
    ...
    def test_exists_miss_w_bound_client(self):
        PATH = f'projects/{self.PROJECT}/jobs/{self.JOB_NAME}'
        conn = _Connection()
        client = _Client(project=self.PROJECT, connection=conn)
        table = _Table()
        job = self._makeOne(self.JOB_NAME, table, [self.SOURCE1], client)
    ...
    def test_cancel_w_bound_client(self):
        PATH = f'projects/{self.PROJECT}/jobs/{self.JOB_NAME}/cancel'
        RESOURCE = self._makeResource()
        conn = _Connection(RESOURCE)
        client = _Client(project=self.PROJECT, connection=conn)
        table = _Table()
